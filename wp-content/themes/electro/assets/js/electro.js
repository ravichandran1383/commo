...
(function($) {
	...
	$('li.dropdown-submenu > a').on('click', function(event) {
		...
		if ( $(this).closest('li.dropdown-submenu').hasClass('open') ) {
			...
			$(this).siblings('.dropdown-menu').find('li.dropdown-submenu').removeClass('open');
		} else {
			$(this).closest('li.dropdown-submenu').removeClass('open');
	...
	$( document ).ready( function() {
		...
		if ( $( window ).width() > 768 ) {
			if( electro_options.enable_sticky_header == '1' && $( "#page" ).find( 'nav' ).hasClass( "navbar-full" ) ) {
		...
		if( electro_options.enable_live_search == '1' ) {
			...
			$( '.navbar-search .product-search-field' ).typeahead( electro_options.typeahead_options,
				{
					name: 'search',
