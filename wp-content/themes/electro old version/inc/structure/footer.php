...
if ( ! function_exists( 'electro_handheld_footer_bar' ) ) {
	...
	function electro_handheld_footer_bar() {
		...
		if ( apply_filters( 'electro_enable_handheld_footer_bar', true ) ) {
			$links = array(
				'my-account' => array(
					'priority' => 10,
...
if ( ! function_exists( 'electro_handheld_footer_bar_account_link' ) ) {
	...
	function electro_handheld_footer_bar_account_link() {
		if ( is_woocommerce_activated() ) {
			echo '<a href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '">' . esc_attr__( 'My Account', 'electro' ) . '</a>';
		}
	}
}
